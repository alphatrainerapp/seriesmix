...
export function UseSavedSessionDialog({
  ...
  const [sessionToConfirm, setSessionToConfirm] = React.useState<SavedSession | null>(null);
  ...
  return (
    <>
      ...
      <AlertDialog open={!!sessionToConfirm} onOpenChange={(open) => !open && setSessionToConfirm(null)}>
        <AlertDialogContent className="sm:max-w-md bg-card rounded-2xl border-none shadow-2xl p-6">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-xl font-black uppercase tracking-tighter italic flex items-center gap-2">
              <AlertTriangle className="h-5 w-5 text-primary" />
              Confirmar Treino
            </AlertDialogTitle>
            <AlertDialogDescription className="space-y-3">
              <span className="text-foreground font-medium text-sm block">
                Deseja carregar a sessão <strong className="text-primary">"{sessionToConfirm?.name}"</strong>?
              </span>
              <span className="text-destructive font-black uppercase text-[10px] tracking-[0.15em] py-2 px-3 bg-destructive/5 rounded-lg border border-destructive/10 block leading-relaxed">
                Atenção: Os exercícios atuais desta aba serão substituídos por esta nova sessão.
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-4 gap-2 sm:gap-0">
            <AlertDialogCancel className="rounded-xl font-bold border-none bg-muted/50 hover:bg-muted text-[11px] uppercase tracking-widest h-12">
              Cancelar
            </AlertDialogCancel>
            <AlertDialogAction 
              onClick={() => sessionToConfirm && handleApply(sessionToConfirm)}
              className="bg-primary hover:bg-primary/90 text-white rounded-xl font-black px-8 shadow-lg shadow-primary/20 border-none h-12 uppercase tracking-widest text-[11px]"
            >
              SIM, CARREGAR TREINO
            </AlertDialogAction>
          </AlertDialogFooter>
